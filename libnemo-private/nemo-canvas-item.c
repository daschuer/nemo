...
/* Private part of the NemoCanvasItem structure. */
struct NemoCanvasItemDetails {
	/* The image, text, font. */
	double x, y;
	GdkPixbuf *pixbuf;
...
static void nemo_canvas_item_text_interface_init (EelAccessibleTextIface *iface);
static GType nemo_canvas_item_accessible_factory_get_type (void);

G_DEFINE_TYPE_WITH_CODE (NemoCanvasItem, nemo_canvas_item, EEL_TYPE_CANVAS_ITEM,
...
static void
nemo_canvas_item_init (NemoCanvasItem *canvas_item)
{
	canvas_item->details = G_TYPE_INSTANCE_GET_PRIVATE ((canvas_item), NEMO_TYPE_CANVAS_ITEM, NemoCanvasItemDetails);
	nemo_canvas_item_invalidate_label_size (canvas_item);
}

static void
nemo_canvas_item_finalize (GObject *object)
{
	NemoCanvasItemDetails *details;

	g_assert (NEMO_IS_CANVAS_ITEM (object));

	details = NEMO_CANVAS_ITEM (object)->details;
	...
	G_OBJECT_CLASS (nemo_canvas_item_parent_class)->finalize (object);
}
...
static void
nemo_canvas_item_invalidate_bounds_cache (NemoCanvasItem *item)
{
...
void
nemo_canvas_item_invalidate_label_size (NemoCanvasItem *item)
{
	...
	nemo_canvas_item_invalidate_bounds_cache (item);
...
static void
nemo_canvas_item_set_property (GObject        *object,
				   guint           property_id,
				   const GValue   *value,
				   GParamSpec     *pspec)
{
	...
	case PROP_EDITABLE_TEXT:
		...
		nemo_canvas_item_invalidate_label_size (item);
	...
	default:
		g_warning ("nemo_canvas_item_set_property on unknown argument");
		return;
	}
...
static void
nemo_canvas_item_get_property (GObject        *object,
				   guint           property_id,
				   GValue         *value,
				   GParamSpec     *pspec)
{
...
void
nemo_canvas_item_set_image (NemoCanvasItem *item,
				GdkPixbuf *image)
{
	NemoCanvasItemDetails *details;	
	
	g_return_if_fail (NEMO_IS_CANVAS_ITEM (item));
...
void 
nemo_canvas_item_set_attach_points (NemoCanvasItem *item,
					GdkPoint *attach_points,
					int n_attach_points)
{
...
void
nemo_canvas_item_set_embedded_text_rect (NemoCanvasItem       *item,
...
void
nemo_canvas_item_set_embedded_text (NemoCanvasItem       *item,
					const char                   *text)
{
 ...
 */
static void
recompute_bounding_box (NemoCanvasItem *canvas_item,
...
static EelIRect
compute_text_rectangle (const NemoCanvasItem *item,
			...
			NemoCanvasItemBoundsUsage usage)
{
	...
	if (NEMO_CANVAS_CONTAINER (EEL_CANVAS_ITEM (item)->canvas)->details->label_position == NEMO_CANVAS_LABEL_POSITION_BESIDE) {
		if (!nemo_canvas_container_is_layout_rtl (NEMO_CANVAS_CONTAINER (EEL_CANVAS_ITEM (item)->canvas))) {
...
void
nemo_canvas_item_update_bounds (NemoCanvasItem *item,
				    double i2w_dx, double i2w_dy)
{
...
static void
nemo_canvas_item_update (EelCanvasItem *item,
			     double i2w_dx, double i2w_dy,
			     gint flags)
{
	nemo_canvas_item_update_bounds (NEMO_CANVAS_ITEM (item), i2w_dx, i2w_dy);

	eel_canvas_item_request_redraw (EEL_CANVAS_ITEM (item));

	EEL_CANVAS_ITEM_CLASS (nemo_canvas_item_parent_class)->update (item, i2w_dx, i2w_dy, flags);
}
...
#define IS_COMPACT_VIEW(container) \
        ((container->details->layout_mode == NEMO_CANVAS_LAYOUT_T_B_L_R || \
...
static void
prepare_pango_layout_width (NemoCanvasItem *item,
			    PangoLayout *layout)
{
	if (nemo_canvas_item_get_max_text_width (item) < 0) {
		pango_layout_set_width (layout, -1);
	} else {
		pango_layout_set_width (layout, floor (nemo_canvas_item_get_max_text_width (item)) * PANGO_SCALE);
		pango_layout_set_ellipsize (layout, PANGO_ELLIPSIZE_END);
	}
}

static void
prepare_pango_layout_for_measure_entire_text (NemoCanvasItem *item,
					      PangoLayout *layout)
{
	...
	container = NEMO_CANVAS_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
...
static void
prepare_pango_layout_for_draw (NemoCanvasItem *item,
			       PangoLayout *layout)
{
	NemoCanvasItemDetails *details;
	NemoCanvasContainer *container;
	...
	if (IS_COMPACT_VIEW (container)) {
		...
		pango_layout_set_height (layout,
					 nemo_canvas_container_get_max_layout_lines_for_pango (container));
	}
}

static void
measure_label_text (NemoCanvasItem *item)
{
	NemoCanvasItemDetails *details;
	NemoCanvasContainer *container;
	...
	if (have_editable) {
		...
		layout_get_size_for_layout (editable_layout,
					    nemo_canvas_container_get_max_layout_lines (container),
					    editable_height_for_entire_text,
...
static void
draw_label_text (NemoCanvasItem *item,
                 cairo_t *cr,
		 EelIRect icon_rect)
{
	NemoCanvasItemDetails *details;
	...
	needs_highlight = details->is_highlighted_for_selection || details->is_highlighted_for_drop;
	is_rtl_label_beside = nemo_canvas_container_is_layout_rtl (container) &&
			      container->details->label_position == NEMO_CANVAS_LABEL_POSITION_BESIDE;
	...
	max_text_width = floor (nemo_canvas_item_get_max_text_width (item));
...
void
nemo_canvas_item_set_is_visible (NemoCanvasItem       *item,
				     gboolean                      visible)
{
	...
	if (!visible) {
		nemo_canvas_item_invalidate_label (item);
	}
}

void
nemo_canvas_item_invalidate_label (NemoCanvasItem     *item)
{
	nemo_canvas_item_invalidate_label_size (item);
...
static void
draw_stretch_handles (NemoCanvasItem *item,
                      cairo_t *cr,
		      const EelIRect *rect)
{
	GtkWidget *widget;
...
real_map_surface (NemoCanvasItem *canvas_item)
{
	EelCanvas *canvas;
...
static cairo_surface_t *
map_surface (NemoCanvasItem *canvas_item)
{
...
static void
draw_embedded_text (NemoCanvasItem *item,
                    cairo_t *cr,
		    int x, int y)
{
	PangoLayout *layout;
...
static void
nemo_canvas_item_draw (EelCanvasItem *item,
                                cairo_t *cr,
...
static PangoLayout *
create_label_layout (NemoCanvasItem *item,
		     const char *text)
{
	PangoLayout *layout;
	PangoContext *context;
	...
	container = NEMO_CANVAS_CONTAINER (canvas_item->canvas);
...
static int
nemo_canvas_item_event (EelCanvasItem *item, GdkEvent *event)
{
	NemoCanvasItem *canvas_item;
	GdkCursor *cursor;
	GdkWindow *cursor_window;
	...
	case GDK_ENTER_NOTIFY:
		...
		if (!canvas_item->details->is_prelit) {
			...
			eel_canvas_item_send_behind (item,
						     NEMO_CANVAS_CONTAINER (item->canvas)->details->rubberband_info.selection_rectangle);
...
static gboolean
hit_test (NemoCanvasItem *canvas_item, EelIRect icon_rect)
{
	NemoCanvasItemDetails *details;
...
static double
nemo_canvas_item_point (EelCanvasItem *item, double x, double y, int cx, int cy,
			    EelCanvasItem **actual_item)
{
	...
	*actual_item = item;
	...
	icon_rect.x1 = cx + 1;
	icon_rect.y1 = cy + 1;
	if (hit_test (NEMO_CANVAS_ITEM (item), icon_rect)) {
		return 0.0;
	} else {
...
static void
nemo_canvas_item_translate (EelCanvasItem *item, double dx, double dy)
{
	NemoCanvasItem *canvas_item;
	NemoCanvasItemDetails *details;
...
void
nemo_canvas_item_get_bounds_for_layout (NemoCanvasItem *canvas_item,
					    double *x1, double *y1, double *x2, double *y2)
{
	...
	nemo_canvas_item_ensure_bounds_up_to_date (canvas_item);
...
void
nemo_canvas_item_get_bounds_for_entire_item (NemoCanvasItem *canvas_item,
						 double *x1, double *y1, double *x2, double *y2)
{
	NemoCanvasItemDetails *details;
	EelIRect *total_rect;
...
static void
nemo_canvas_item_bounds (EelCanvasItem *item,
			     double *x1, double *y1, double *x2, double *y2)
{
	NemoCanvasItem *canvas_item;
	NemoCanvasItemDetails *details;
...
static void
nemo_canvas_item_ensure_bounds_up_to_date (NemoCanvasItem *canvas_item)
{
	NemoCanvasItemDetails *details;
...
EelDRect
nemo_canvas_item_get_icon_rectangle (const NemoCanvasItem *item)
{
	EelDRect rectangle;
	double pixels_per_unit;
	gint width, height;
...
EelDRect
nemo_canvas_item_get_text_rectangle (NemoCanvasItem *item,
					 gboolean for_layout)
{
	/* FIXME */
	EelIRect icon_rectangle;
...
void
nemo_canvas_item_set_show_stretch_handles (NemoCanvasItem *item,
...
static gboolean
hit_test_stretch_handle (NemoCanvasItem *item,
...
gboolean
nemo_canvas_item_hit_test_stretch_handles (NemoCanvasItem *item,
					       gdouble world_x,
					       gdouble world_y,
					       GtkCornerType *corner)
{
	...
	g_return_val_if_fail (NEMO_IS_CANVAS_ITEM (item), FALSE);
...
/* nemo_canvas_item_hit_test_rectangle
 *
 * Check and see if there is an intersection between the item and the
 * canvas rect.
 */
gboolean
nemo_canvas_item_hit_test_rectangle (NemoCanvasItem *item, EelIRect icon_rect)
{
	g_return_val_if_fail (NEMO_IS_CANVAS_ITEM (item), FALSE);
...
const char *
nemo_canvas_item_get_editable_text (NemoCanvasItem *canvas_item)
{
	g_return_val_if_fail (NEMO_IS_CANVAS_ITEM (canvas_item), NULL);

	return canvas_item->details->editable_text;
}

void
nemo_canvas_item_set_renaming (NemoCanvasItem *item, gboolean state)
{
	g_return_if_fail (NEMO_IS_CANVAS_ITEM (item));
...
double
nemo_canvas_item_get_max_text_width (NemoCanvasItem *item)
{
	EelCanvasItem *canvas_item;
	NemoCanvasContainer *container;
	...
	canvas_item = EEL_CANVAS_ITEM (item);
	container = NEMO_CANVAS_CONTAINER (canvas_item->canvas);
    if (nemo_canvas_container_is_tighter_layout (container)) {
       return MAX_TEXT_WIDTH_TIGHTER * canvas_item->canvas->pixels_per_unit;
    } else {
        if (container->details->label_position == NEMO_CANVAS_LABEL_POSITION_BESIDE) {
           if (container->details->layout_mode == NEMO_CANVAS_LAYOUT_T_B_L_R ||
               container->details->layout_mode == NEMO_CANVAS_LAYOUT_T_B_R_L) {
...
void
nemo_canvas_item_set_entire_text (NemoCanvasItem       *item,
...
static void
nemo_canvas_item_class_init (NemoCanvasItemClass *class)
{
	...
	object_class->finalize = nemo_canvas_item_finalize;
	object_class->set_property = nemo_canvas_item_set_property;
	object_class->get_property = nemo_canvas_item_get_property;
	...
	item_class->update = nemo_canvas_item_update;
	item_class->draw = nemo_canvas_item_draw;
	item_class->point = nemo_canvas_item_point;
	item_class->translate = nemo_canvas_item_translate;
	item_class->bounds = nemo_canvas_item_bounds;
	item_class->event = nemo_canvas_item_event;

	atk_registry_set_factory_type (atk_get_default_registry (),
				       NEMO_TYPE_CANVAS_ITEM,
				       nemo_canvas_item_accessible_factory_get_type ());

	g_type_class_add_private (class, sizeof (NemoCanvasItemDetails));
}

static GailTextUtil *
nemo_canvas_item_get_text (GObject *text)
{
	return NEMO_CANVAS_ITEM (text)->details->text_util;
}

static void
nemo_canvas_item_text_interface_init (EelAccessibleTextIface *iface)
{
	iface->get_text = nemo_canvas_item_get_text;
}

/* ============================= a11y interfaces =========================== */

static const char *nemo_canvas_item_accessible_action_names[] = {
        "open",
        "menu",
        NULL
};

static const char *nemo_canvas_item_accessible_action_descriptions[] = {
        "Open item",
        "Popup context menu",
        NULL
};
...
typedef struct {
	...
	char *description;
} NemoCanvasItemAccessiblePrivate;

typedef struct {
	NemoCanvasItem *item;
	gint action_number;
} NemoCanvasItemAccessibleActionContext;

typedef struct {
	EelCanvasItemAccessible parent;
	NemoCanvasItemAccessiblePrivate *priv;
} NemoCanvasItemAccessible;

typedef struct {
	EelCanvasItemAccessibleClass parent_class;
} NemoCanvasItemAccessibleClass;

#define GET_ACCESSIBLE_PRIV(o) ((NemoCanvasItemAccessible *) o)->priv;

/* accessible AtkAction interface */
static gboolean
nemo_canvas_item_accessible_idle_do_action (gpointer data)
{
	NemoCanvasItem *item;
	NemoCanvasItemAccessibleActionContext *ctx;
	NemoCanvasIcon *icon;
	NemoCanvasContainer *container;
	GList* selection;
	GList file_list;
...
static gboolean
nemo_canvas_item_accessible_do_action (AtkAction *accessible,
					   int i)
{
	NemoCanvasItem *item;
	NemoCanvasItemAccessibleActionContext *ctx;
	...
	item = NEMO_CANVAS_ITEM (atk_gobject_accessible_get_object (ATK_GOBJECT_ACCESSIBLE (accessible)));
	if (!item) {
		return FALSE;
	}
	...
	case ACTION_MENU:
		...
		ctx = g_new (NemoCanvasItemAccessibleActionContext, 1);
		ctx->action_number = i;
		ctx->item = item;
		g_queue_push_head (container->details->a11y_item_action_queue, ctx);
		if (container->details->a11y_item_action_idle_handler == 0) {
			container->details->a11y_item_action_idle_handler = g_idle_add (nemo_canvas_item_accessible_idle_do_action, container);
		}
		break;
        default :
                g_warning ("Invalid action passed to NemoCanvasItemAccessible::do_action");
                return FALSE;
        }
...
static int
nemo_canvas_item_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
nemo_canvas_item_accessible_action_get_description (AtkAction *accessible,
							int i)
{
	NemoCanvasItemAccessiblePrivate *priv;
	...
	if (priv->action_descriptions[i]) {
		...
		return nemo_canvas_item_accessible_action_descriptions[i];
	}
}

static const char *
nemo_canvas_item_accessible_action_get_name (AtkAction *accessible, int i)
{
	g_assert (i < LAST_ACTION);

	return nemo_canvas_item_accessible_action_names[i];
}

static const char *
nemo_canvas_item_accessible_action_get_keybinding (AtkAction *accessible,
						       int i)
{
	g_assert (i < LAST_ACTION);
...
static gboolean
nemo_canvas_item_accessible_action_set_description (AtkAction *accessible,
							int i,
...
static void
nemo_canvas_item_accessible_action_interface_init (AtkActionIface *iface)
{
	iface->do_action = nemo_canvas_item_accessible_do_action;
	iface->get_n_actions = nemo_canvas_item_accessible_get_n_actions;
	iface->get_description = nemo_canvas_item_accessible_action_get_description;
	iface->get_keybinding = nemo_canvas_item_accessible_action_get_keybinding;
	iface->get_name = nemo_canvas_item_accessible_action_get_name;
	iface->set_description = nemo_canvas_item_accessible_action_set_description;
}

static const gchar *
nemo_canvas_item_accessible_get_name (AtkObject *accessible)
{
	NemoCanvasItem *item;
...
static const gchar*
nemo_canvas_item_accessible_get_description (AtkObject *accessible)
{
	NemoCanvasItem *item;
...
static AtkObject *
nemo_canvas_item_accessible_get_parent (AtkObject *accessible)
{
	NemoCanvasItem *item;
...
static int
nemo_canvas_item_accessible_get_index_in_parent (AtkObject *accessible)
{
	NemoCanvasItem *item;
	NemoCanvasContainer *container;
	GList *l;
...
static const gchar *
nemo_canvas_item_accessible_get_image_description (AtkImage *image)
{
	NemoCanvasItemAccessiblePrivate *priv;
	NemoCanvasItem *item;
	NemoCanvasIcon *icon;
	...
	if (priv->image_description) {
		...
		icon = item->user_data;
		container = NEMO_CANVAS_CONTAINER (EEL_CANVAS_ITEM (item)->canvas);
		description = nemo_canvas_container_get_icon_description (container, icon->data);
...
static void
nemo_canvas_item_accessible_get_image_size
	(AtkImage *image, 
	 gint     *width,
	 gint     *height)
{
...
static void
nemo_canvas_item_accessible_get_image_position
	(AtkImage		 *image,
	 gint                    *x,
	 gint	                 *y,
...
static gboolean
nemo_canvas_item_accessible_set_image_description (AtkImage    *image,
...
static void
nemo_canvas_item_accessible_image_interface_init (AtkImageIface *iface)
{
	iface->get_image_description = nemo_canvas_item_accessible_get_image_description;
	iface->set_image_description = nemo_canvas_item_accessible_set_image_description;
	iface->get_image_size        = nemo_canvas_item_accessible_get_image_size;
	iface->get_image_position    = nemo_canvas_item_accessible_get_image_position;
}

/* accessible text interface */
static gint
nemo_canvas_item_accessible_get_offset_at_point (AtkText	 *text,
						     gint           x,
						     gint           y,
						     AtkCoordType coords)
{
...
static void
nemo_canvas_item_accessible_get_character_extents (AtkText	   *text,
						       gint	   offset,
						       gint	   *x,
						       gint	   *y,
						       gint	   *width,
...
static void
nemo_canvas_item_accessible_text_interface_init (AtkTextIface *iface)
{
	...
	iface->get_character_extents   = nemo_canvas_item_accessible_get_character_extents;
	iface->get_offset_at_point     = nemo_canvas_item_accessible_get_offset_at_point;
}

static GType nemo_canvas_item_accessible_get_type (void);

G_DEFINE_TYPE_WITH_CODE (NemoCanvasItemAccessible,
			 nemo_canvas_item_accessible,
			 eel_canvas_item_accessible_get_type (),
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_IMAGE,
						nemo_canvas_item_accessible_image_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_TEXT,
						nemo_canvas_item_accessible_text_interface_init)
			 G_IMPLEMENT_INTERFACE (ATK_TYPE_ACTION,
						nemo_canvas_item_accessible_action_interface_init));

static AtkStateSet*
nemo_canvas_item_accessible_ref_state_set (AtkObject *accessible)
{
...
static void
nemo_canvas_item_accessible_finalize (GObject *object)
{
	NemoCanvasItemAccessiblePrivate *priv;
	int i;
	...
	g_free (priv->description);
        ...
        G_OBJECT_CLASS (nemo_canvas_item_accessible_parent_class)->finalize (object);
}

static void
nemo_canvas_item_accessible_initialize (AtkObject *accessible,
					    gpointer widget)
{
	ATK_OBJECT_CLASS (nemo_canvas_item_accessible_parent_class)->initialize (accessible, widget);

	atk_object_set_role (accessible, ATK_ROLE_CANVAS);
}

static void
nemo_canvas_item_accessible_class_init (NemoCanvasItemAccessibleClass *klass)
{
	AtkObjectClass *aclass = ATK_OBJECT_CLASS (klass);
	GObjectClass *oclass = G_OBJECT_CLASS (klass);

	oclass->finalize = nemo_canvas_item_accessible_finalize;

	aclass->initialize = nemo_canvas_item_accessible_initialize;

	aclass->get_name = nemo_canvas_item_accessible_get_name;
	aclass->get_description = nemo_canvas_item_accessible_get_description;
	aclass->get_parent = nemo_canvas_item_accessible_get_parent;
	aclass->get_index_in_parent = nemo_canvas_item_accessible_get_index_in_parent;
	aclass->ref_state_set = nemo_canvas_item_accessible_ref_state_set;

	g_type_class_add_private (klass, sizeof (NemoCanvasItemAccessiblePrivate));
}

static void
nemo_canvas_item_accessible_init (NemoCanvasItemAccessible *self)
{
	self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self, nemo_canvas_item_accessible_get_type (),
						  NemoCanvasItemAccessiblePrivate);
}

/* dummy typedef */
typedef AtkObjectFactory      NemoCanvasItemAccessibleFactory;
typedef AtkObjectFactoryClass NemoCanvasItemAccessibleFactoryClass;

G_DEFINE_TYPE (NemoCanvasItemAccessibleFactory, nemo_canvas_item_accessible_factory,
	       ATK_TYPE_OBJECT_FACTORY);

static AtkObject *
nemo_canvas_item_accessible_factory_create_accessible (GObject *for_object)
{
	AtkObject *accessible;
	NemoCanvasItem *item;
	GString *item_text;

	item = NEMO_CANVAS_ITEM (for_object);
	g_assert (item != NULL);
	...
	accessible = g_object_new (nemo_canvas_item_accessible_get_type (), NULL);
...
static GType
nemo_canvas_item_accessible_factory_get_accessible_type (void)
{
	return nemo_canvas_item_accessible_get_type ();
}

static void
nemo_canvas_item_accessible_factory_init (NemoCanvasItemAccessibleFactory *self)
{
}

static void
nemo_canvas_item_accessible_factory_class_init (NemoCanvasItemAccessibleFactoryClass *klass)
{
	klass->create_accessible = nemo_canvas_item_accessible_factory_create_accessible;
	klass->get_accessible_type = nemo_canvas_item_accessible_factory_get_accessible_type;
}

void
nemo_canvas_item_set_tooltip_text            (NemoCanvasItem *item, const gchar *text)
{
